...
for /f "tokens=1-2 delims=:" %%a in ("%TIME%") do (
    title [ ETERNAL TWEAKER ^| %USERNAME% ^| Launched: %%a:%%b ]
)
CHCP 65001 >nul
...
echo %c%Config Content Delivery%w%
...
echo %c%Disable Reserved Storage%w%
start "C:\Windows\System32\Dism.exe" "/Online /Set-ReservedStorageState /State:Disabled"

echo %c%Disanle Scheduled Tasks%w%
schtasks /change /tn "\Microsoft\Windows\Application Experience\PcaPatchDbTask" /Disable
schtasks /change /tn "\Microsoft\Windows\AppxDeploymentClient\UCPD velocity" /Disable
schtasks /change /tn "c" /Disable
...
echo %c%Hide Unused Security Pages%w%
...
echo %c%Disable LLMNR Protocol%w%
reg add "HKLM\SOFTWARE\Policies\Microsoft\Windows NT\DNSClient" /v "EnableMulticast" /t REG_DWORD /d "0" /f

echo %c%Share netowork tweaks...%w%
reg add "HKLM\SYSTEM\CurrentControlSet\Services\LanmanWorkstation\Parameters" /v "DisableBandwithThrottling" /t REG_DWORD /d "1" /f
reg add "HKLM\SYSTEM\CurrentControlSet\Services\LanManServer\Parameters" /v "RestrictNullSessAcess" /t REG_DWORD /d "1" /f 
reg add "HKLM\SYSTEM\CurrentControlSet\Control\Lsa" /v "RestrictAnonymous" /t REG_DWORD /d "1" /f 

echo %c%Configure Automatic Maintenance%w%
reg add "HKLM\SOFTWARE\Policies\Microsoft\Windows\Task Scheduler\Maintenance" /v "WakeUp" /t REG_DWORD /d "0" /f

echo %c%Configure the Multimedia Class Scheduler Service%w%
reg add "HKLM\SOFTWARE\Microsoft\Windows NT\CurrentVersion\Multimedia\SystemProfile" /v "SystemResponsiveness" /t REG_DWORD /d "10" /f 

echo %c%Disbale Automatic Folder Disovery%w%
:: Change to match the setting name (e.g., Sleep, Indexing, etc.)
...
echo %c%Disable Background Apps%w%
reg add "HKCU\SOFTWARE\Microsoft\Windows\CurrentVersion\BackgroundAccessApplications" /v "GlobalUserDisabled" /t REG_DWORD /d "1" /f 
reg add "HKCU\SOFTWARE\Microsoft\Windows\CurrentVersion\Search" /v "BackgroundAppGlobalToggle" /t REG_DWORD d "0" /f 

echo %c%Disable Fault Tolerant Heap%w%
start "C:\Windows\System32\rundll32.exe" "fthsvc.dll,FthSysprepSpecialize"
reg add "HKLM\SOFTWARE\Microsoft\FTH" /v "Enabled" /t REG_DWORD /d "0" /f 

echo %c%Disable Game Bar%w%
...
echo %c%Disable Modern Standby SleepStudy%w%
start "C:\Windows\System32\wevutil.exe" "set-log 'Microsoft-Windows-SleepStudy/Diagnostic' /e:false"
start "C:\Windows\System32\wevutil.exe" "set-log 'Microsoft-Windows-Kernel-Processor-Power/Diagnostic' /e:false"
start "C:\Windows\System32\wevutil.exe" "set-log 'Microsoft-Windows-UserModePowerService/Diagnostic' /e:false"
schtasks /change /tn "\Microsoft\Windows\Power Efficiency Diagnostics\AnalyzeSystem" /Disable

echo %c%Respect Power Modes Windows Search Indexing%w%
reg add "HKLM\Software\Microsoft\Windows Search\Gather\Windows\SystemIndex" /v "RespectPowerModes" /t REG_DWORD /d "1" /f 

echo %c%Disable Paging Settings%w%
reg add "HKLM\SYSTEM\CurrentControlSet\Control\Session Manager\Memory Management" /v "DisablePagingExecutive" /t REG_DWORD /d "1" /f 
reg add "HKLM\SYSTEM\CurrentControlSet\Control\Session Manager\Memory Management" /v "DisablePageCombining" /t REG_DWORD /d "1" /f 

echo %c%Disable Service Host Splitting%w%
powershell -Command "& 'C:\eternal\scripts\Disable Service Host Split.ps1'"

echo %c%Optimize NTFS%w%
start "C:\Windows\System32\fsutil.exe" "behavior and disablelastaccess 1"
start "C:\Windows\System32\fsutil.exe" "8dot3name set 1"

echo %c%Prioritize Foreground Applications%w%
reg add "HKLM\SYSTEM\CurrentControlSet\Control\PriorityControl" /v "Win32PrioritySeparation" /t REG_DWORD /d "38" /f 

echo %c%Configure App Permissions%w%
reg add "HKLM\SOFTWARE\Microsoft\Windows\CurrentVersion\CapabilityAccessManager\ConsentStore\appDiagnostics" /v "Value" /t REG_SZ /d "Deny" /f 
reg add "HKLM\SOFTWARE\Microsoft\Windows\CurrentVersion\CapabilityAccessManager\ConsentStore\location" /v "Value" /t REG_SZ /d "Deny" /f 
reg add "HKLM\SOFTWARE\Microsoft\Windows\CurrentVersion\CapabilityAccessManager\ConsentStore\userAccountInformation" /v "Value" /t REG_SZ /d "Deny" /f 

echo %c%Configure Windows Media Player%w%
...
echo %c%Disable Activity Feed%w%
reg add "HKLM\SOFTWARE\Policies\Microsoft\Windows\System" /v "EnableActivityFeed" /t REG_DWORD /d "0" /f 

echo %c%Disable App Launch Tracking%w%
reg add "HKCU\SOFTWARE\Microsoft\Windows\CurrentVersion\Explorer\Advanced" /v "Start_TrackProgs" /t REG_DWORD /d "0" /f 

echo %c%Disable Device Health Attestation Monitoring and Reporting%w%
...
echo %c%Disable Experimentation%w%
reg add "HKLM\SOFTWARE\Microsoft\PolicyManager\default\System\AllowExperimentation" /v "Value" /t REG_DWORD /d "0" /f 

echo %c%Disable Lockscreen Camera%w%
reg add "HKLM\SOFTWARE\Policies\Microsoft\Windows\Personalization" /v "NoLockScreenCamera" /t REG_DWORD /d "1" /f 

echo %c%Disable Online Speech Recognition%w%
reg add "HKCU\SOFTWARE\Microsoft\Speech_OneCore\Settings\OnlineSpeechPrivacy" /v "HasAccepted" /t REG_DWORD /d "0" /f 

echo %c%Disable Program Compatibility Assistant%w%
...
echo %c%Disable Performance Track%w%
reg add "HKLM\SOFTWARE\Policies\Microsoft\Windows\WDI\{9c5a40da-b965-4fc3-8781-88dd50a6299d}" /v "ScenarioExecutionEnabled" /t REG_DWORD /d "0" /f 

echo %c%Disable OOBE Privacy Experience%w%
reg add "HKLM\SOFTWARE\Policies\Microsoft\Windows\OOBE" /v "DisablePrivacyExperience" /t REG_DWORD /d "1" /f 

echo %c%Disable Recall Snapshots%w%
...
echo %c%Disable RSoP Logging%w%
reg add "HKLM\SOFTWARE\Policies\Microsoft\Windows\System" /v "RSoPLogging" /t REG_DWORD /d "0" /f 

echo %c%Disable Automatic Updates Of Speech Data%w%
reg add "HKLM\SOFTWARE\Policies\Microsoft\Speech" /v "AllowSpeechModeUpdate" /t REG_DWORD /d "0" /f 

echo %c%Disable Tailored Experiences%w%
reg add "HKCU\SOFTWARE\Microsoft\Windows\CurrentVersion\Privacy" /v "TailoredExperiencesWithDiagnosticEnabled" /t REG_DWORD /d "0" /f 
reg add "HKCU\SOFTWARE\Policies\Microsoft\Windows\CloudContent" /v "DisableTailoredExperiencesWithDiagnosticDate" /t REG_DWORD /d "1" /f 

echo %c%Disable Most Frequently Used Applications%w%
reg add "HKCU\SOFTWARE\Microsoft\Windows\CurrentVersion\Policies\Explorer" /v "NoInstrumentation" /t REG_DWORD /d "1" /f 

echo %c%Disable Website Acess to Language List%w%
reg add "HKCU\Control Panel\International\User Profile" /v "HttpAcceptLanguageOptOut" /t REG_DWORD /d "1" /f 

echo %c%Disable Windows Error Reporting%w%
...
reg add "HKCU\SOFTWARE\Policies\Microsoft\Windows\Windows Error Reporting" /v "ShowUI" /t REG_DWORD /d "0" /f 
...
reg add "HKLM\SOFTWARE\Policies\Microsoft\Windows\DeviceInstall\Settings" /v "DisableSendGenericDriverNotFoundToWER" /t REG_DWORD /d "1" 
...
echo %c%Disable Users to Be Non-local%w%
reg add "HKLM\SOFTWARE\Microsoft\Windows\CurrentVersion\Policies\System" /v "NoConnectedUser" /t REG_DWORD /d "1" /f 

echo %c%Disallow Upload and Publish of User Activities%w%
reg add "HKLM\SOFTWARE\Policies\Microsoft\Windows\System" /v "UploadUserActivites" /t REG_DWORD /v "0" /f 
reg add "HKLM\SOFTWARE\Policies\Microsoft\Windows\System" /v "PublishUserActivities" /t REG_DWORD /v "0" /f 

echo %c%Configure Search on the Taskbar%w%
reg add "HKCU\SOFTWARE\Microsoft\Windows\CurrentVersion\Search" /v "BingSearchEnabled" /t REG_DWORD /v "0" /f 
reg add "HKCU\SOFTWARE\Microsoft\Windows\CurrentVersion\SearchSettings" /v "IsDeviceSearchHistoryEnabled" /t REG_DWORD /v "0" /f 
reg add "HKCU\SOFTWARE\Microsoft\Windows\CurrentVersion\SearchSettings" /v "IsMSACloudSearchEnabled" /t REG_DWORD /v "0" /f 
...
reg add "HKCU\SOFTWARE\Microsoft\Windows\CurrentVersion\Search" /t REG_DWORD /v "SearchboxTaskbarMode" /d "1" /f 

echo %c%Disable Advertising ID%w%
reg add "HKCU\SOFTWARE\Microsoft\Windows\CurrentVersion\AdvertisingInfo" /v "Enabled" /t REG_DWORD /d "0" /f 
reg add "HKLM\Software\Policies\Microsoft\Windows\AdvertisingInfo" /v "DisabledByGroupPolicy" /t REG_DWORD /d "1" /f 

echo %c%Disable Sync Provider Notifications%w%
reg add "HKCU\SOFTWARE\Microsoft\Windows\CurrentVersion\Explorer\Advanced" /v "ShowSyncProviderNotifications" /t REG_DWORD /d "0" /f 

echo %c%Disable NVIDIA Control Panel Telemetry%w%
reg add "HKCU\Software\NVIDIA Corporation\NVControlPanel2\Client" /v "OptInOrOutPreference" /t REG_DWORD /d "0" /f 

echo %c%Disable Office Telemetry%w%
reg add "HKCU\Software\Policies\Microsoft\office\16.0\common" /v "sendcustomerdata" /t REG_DWORD /v "0" /f 
reg add "HKCU\Software\Policies\Microsoft\office\16.0\common" /v "qmenable" /t REG_DWORD /v "0" /f 
reg add "HKCU\Software\Policies\Microsoft\office\common\clienttelemetry"/v "sendtelemetry" /t REG_DWORD /v "3" /f 

echo %c%Disable Settings Sync%w%
reg add "HKLM\SOFTWARE\Policies\Microsoft\Windows\SettingSync" /v "DisableSettingSync" /t REG_DWORD /v "2" /f 
reg add "HKLM\SOFTWARE\Policies\Microsoft\Windows\SettingSync" /v "DisableSettingsSyncUserOverride" /t REG_DWORD /v "1" /f 
reg add "HKLM\SOFTWARE\Policies\Microsoft\Windows\SettingSync" /v "DisableSyncOnPaidNetwork" /t REG_DWORD /v "1" /f 
reg add "HKLM\SOFTWARE\Policies\Microsoft\Windows\SettingSync" /v "DisableWindowsSettingSync" /t REG_DWORD /v "2" /f 
reg add "HKCU\SOFTWARE\Microsoft\Windows\CurrentVersion\SettingSync\Groups\Personalization" /v "Enabled" /t REG_DWORD /v "0" /f
reg add "HKCU\SOFTWARE\Microsoft\Windows\CurrentVersion\SettingSync\Groups\BrowserSettings" /v "Enabled" /t REG_DWORD /v "0" /f 
reg add "HKCU\SOFTWARE\Microsoft\Windows\CurrentVersion\SettingSync\Groups\Credentials" /v "Enabled" /t REG_DWORD /v "0" /f 
reg add "HKCU\SOFTWARE\Microsoft\Windows\CurrentVersion\SettingSync\Groups\Accessibility" /v "Enabled" /t REG_DWORD /v "0" /f 
reg add "HKCU\SOFTWARE\Microsoft\Windows\CurrentVersion\SettingSync\Groups\Windows" /v "Enabled" /t REG_DWORD /d "0" /f 
reg add "HKCU\SOFTWARE\Microsoft\Windows\CurrentVersion\SettingSync" /v "SyncPolicy" /t REG_DWORD /d "5" /f 

echo %c%Disable Suggested Ways To Finish Settings Up Your Device%w%
reg add "HKCU\SOFTWARE\Microsoft\Windows\CurrentVersion\UserProfileEngagement" /v "ScoobeSystemSettingEnabled" /t REG_DWORD /d "0" /f 

echo %c%Disallow Message Service Cloud Sync%w%
reg add "HKLM\SOFTWARE\Policies\Microsoft\Windows\Messaging" /v "AllowMessageSync" /t REG_DWORD /d "0" /f 

echo %c%Disable Key Management System Telemetry%w%
reg add "HKLM\SOFTWARE\Policies\Microsoft\Windows NT\CurrentVersion\Software Protection Platform" /v "NoGenTicket" /t REG_DWORD /d "1" /f 

echo %c%Disable CEIP%w%
reg add "HKLM\SOFTWARE\Policies\Microsoft\AppV\CEIP" /v "CEIPEnable" /t REG_DWORD /d "0" /f 
reg add "HKLM\SOFTWARE\Policies\Microsoft\SQMClient\Windows" /v "CEIPEnable" /t REG_DWORD /d "0" /f

echo %c%Disable Diagnostic Tracing%w%
reg add "HKLM\SYSTEM\CurrentControlSet\Control\Diagnostics\Performance" /v "DisableDiagnosticTracing" /t REG_DWORD /d "1" /f 

echo %c%Disable .NET CLI Telemetry%w%
setx DOTNET_CLI_TELEMETRY_OPTOUT 1

echo %c%Disable Input Telemetry%w%
...
echo %c%Disallow Telemtry and Data Collection%w%
net stop DiagTrack
...
reg add "HKLM\Software\Microsoft\Windows\CurrentVersion\Diagnostics\DiagTrack\EventTranscriptKey" /v "MiniTraceSlotEnabled" /t REG_DWORD /d "0"
